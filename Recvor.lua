-- ============================================
--  RECVOR HUB Beta | Smooth Animations + Settings
...
ContentBarImageColor3 = Color3.fromRGB(60, 60, 80)
...
local function AnimateIn(element, delayTime)
    ...
    task.delay(delayTime, function()
        TweenService:Create(element, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
...
CreateToggle(CurrentLang.flyMobile, "FlyMobile", function(isOn)
...
JerkBtn.MouseButton1Click:Connect(function()
    GiveJerkOffTool()
    Notify("😏 Jerk Off Tool", CurrentLang.jerkAdded)
end)
...
local function ApplyTransparency()
    local trans = Settings.Transparency / 100
...
function StartMobileFly()
    if MobileFlyActive then return end
    ...
    JoystickBg.Parent = MobileFlyGui
    ...
    DownBtn.Position = UDim2.new(1, -80, 1, -75)
    ...
    local function handleJoystick(input)
        ...
        if dist > 0 then
            ...
            moveDir = (camera.CFrame.LookVector * (-ny / maxDist) + camera.CFrame.RightVector * (nx / maxDist)) * speed
        else
            JoystickKnob.Position = UDim2.new(0.5, -20, 0.5, -20)
    ...
    MobileFlyConnection = RunService.Heartbeat:Connect(function()
        ...
        local totalDir = moveDir + Vector3.new(0, verticalDir * Settings.FlySpeed * 0.5, 0)
...
function GiveJerkOffTool()
    ...
    handle.Name = "Handle"
    handle.Size = Vector3.new(0.8, 2.5, 0.8)
    handle.BrickColor = BrickColor.new("Pastel Brown")
    ...
    tool.Equipped:Connect(function()
        ...
        tool:SetAttribute("Particle", particle)
    end)
    ...
    tool.Activated:Connect(function()
        local anim = Instance.new("Animation")
        anim.AnimationId = "rbxassetid://507765644"
        local track = Humanoid:LoadAnimation(anim)
        track:Play()
        
        local particle = tool:GetAttribute("Particle")
        if particle then
            particle.Rate = 80
            task.wait(0.5)
            particle.Rate = 15
...
task.delay(0.6, function()
    Notify("🔥 RECVOR HUB v3.0", CurrentLang.welcome .. " " .. CurrentLang.menuToggle)
end)

print("✅ RECVOR HUB v3.0 loaded!")
